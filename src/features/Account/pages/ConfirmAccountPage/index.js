...
function ConfirmAccountPage(props) {
    ...
    const handleConfirmAccount = async (user) => { 
        const { username, password,otpValue } = account;
        console.log('is try',account);
          try{
            dispatch(setLoading(true)); 
                const response = await loginApi.confirmAccount(account.username, user.otpValue);
        ...
        <div className="confirm-account-page">
            <div className="main">
                ...
                <Formik
                    ...
                    onSubmit={(user)=>handleConfirmAccount(user)}
